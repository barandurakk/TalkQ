  ...
  componentDidMount() {
    ...
    socket.on("getMessage", (message) => {
      ...
      this.props.updateConversations({
        ...
        friendAvatar: message.userAvatar,
      });
    });
        ...
        <div className="notification-list">
          {friendRequestList && friendRequestList.length > 0 ? (
            friendRequestList.map((request) => {
              return <RequestItem request={request} key={request.requester} username={auth.name} />;
            })
          ) : (
    ...
    return (
      <Fragment>
        <div className="notification-button-wrapper">
          <span className="notification-button" onClick={() => this.handleNotificationButton()}>
...
export default connect(mapStateToProps, {
  ...
  updateFriends,
  deleteConversation,
})(withSnackbar(Notifications, alertOptions));
