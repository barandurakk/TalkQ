  ...
  return (
    <div className="bigAvatar-container">
      ...
      <div className="bigAvatar-actions">
        <span onClick={() => setShowBigAvatar(false)}>Close</span>
        <span onClick={() => onEditImage()}>Change</span>
      </div>
    </div>
  );
};
    ...
    <div className="userDetail-container">
      <div className="userDetail-top">
        <div className="userDetail">
          <span onClick={() => setShowBigAvatar(!showBigAvatar)}>
            <img src={userDetails.pictureUrl} className="userAvatar" alt="User Avatar" />
          </span>
          <input
            type="file"
            id="imageInput"
            hidden="hidden"
            onChange={(event) => onImageChange(event, dispatch)}
          />
      ...
      <div className="userId-container">
        <span className="userId">
          ID:<strong> {userDetails._id}</strong>
        </span>
        <CopyToClipboard text={userDetails._id} onCopy={() => openSnackbar("User ID Copied!")}>
      ...
      {showBigAvatar ? renderBigAvatar(userDetails.pictureUrl, setShowBigAvatar) : null}
    </div>
  ) : (
    <div className="loading-container">
      <Loader type="TailSpin" color="#077b70" height={100} width={100} visible={loading} />
    </div>
  );
};
